...
description: "Three families of fast-sampling methods, derived rather than listed. Including the result that surprises people: a mathematically better solver can be twice as bad as a crude one when you only have two network calls to spend."
...
This is why the sub-10-NFE regime needed genuinely different ideas rather than better quadrature — and why papers reporting one-step results are not simply doing the same thing harder.
...
true_end = sample(probe.clone(), 800, heun_step)

average_direction = (true_end - probe) / (T_END - T_START)     # displacement / time
initial_direction = velocity(probe, torch.tensor(T_START))

actual_error = (probe + (T_END - T_START) * initial_direction - true_end).norm(dim=-1)
...
There are three increasingly general versions of that idea.
