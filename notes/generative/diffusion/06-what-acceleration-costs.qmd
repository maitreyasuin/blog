...
description: "A paper reports 50 steps to 4 and calls it a 12× speedup. Your service will not see 12×. This post is about the gap: the things you pay for besides network calls, the accelerations that cancel each other out, and the failure modes that only appear once the step count is low."
...
That barely matters at 50 steps, when denoising dominates everything. It matters enormously at 4. The arithmetic is Amdahl's law, and it's worth doing explicitly because the intuition is unreliable:
...
print(f"   {steps * 2} actual network evaluations")
print(f"\nversus an unguided {steps * 2}-step sampler: identical network cost, "
      f"and one of them is described with a number twice as large.")
```
...
## Failure modes that only appear at low step counts

Six things that were fine at 50 steps and are not fine at 4.
...
Three things this series deliberately left alone. **Video**, where the step count multiplies by frame count and few-step generation is the difference between a demo and a product. **The systems layer** — quantization, kernel work, and distributed inference — which is orthogonal to everything here and often the cheapest win available. And **flow matching itself**, which has been used throughout as the convenient formalism without being derived properly; that derivation is its own post and the natural companion to this one.
