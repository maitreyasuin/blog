...
where $x$ is a clean sample, $\epsilon\sim\mathcal N(0,I)$ is Gaussian noise, and $t\in[0,1]$ is the noise level. At $t=0$ the state is data; at $t=1$ it is pure noise. The exact coefficients differ across model families, but the local-versus-finite-step issue studied below does not.
...
def fraction_near_a_blob(x):
    """What share of these points landed within 3 standard deviations of a blob?
    For real data this should be ~99.7%."""
...
- **How much it averages over depends on the noise level.** At $t=1$ the input carries no information about any particular sample, so the answer is the mean of the entire dataset — provably, with zero spread across 50,000 different inputs.
