---
title: "Ten Tensor Problems Where Shape Is Not Enough"
description: "A closing self-check on the tensor fundamentals: ten small problems where an answer can be semantically wrong despite a plausible shape — or mathematically right but unusable because of layout, numerics, device placement, or synchronization cost."
categories: [tensors, drill]
date: 2026-07-20
depth: "Note"
format:
  html:
...
Read each problem, close the page, write your version. Then open the solution. If you want the reasoning behind any of them, each links back to the post that develops it: [strides and views](01-strides-views-contiguity.qmd), [broadcasting and einsum](02-broadcasting-reductions-einsum.qmd), [gather and indexing](03-gather-scatter-indexing.qmd), [numerical stability](04-numerical-stability.qmd), and [the GPU cost model](05-how-the-gpu-runs-your-code.qmd).
...
## 1. Dot product without `torch.dot`

```python
a, b = torch.randn(16), torch.randn(16)
# produce the scalar dot product
```
...
a, b = torch.randn(16), torch.randn(16)
c = (a * b).sum(dim=-1)
torch.testing.assert_close(c, torch.dot(a, b))
print("elementwise-multiply-then-reduce matches torch.dot  ✓")
```
...
```{python}
for B, D in [(4, 16), (8, 8)]:
    x, y = torch.randn(B, D), torch.randn(B, D)
    try:
        print(f"B={B}, D={D}: a @ b -> {tuple((x @ y).shape)}"
              + ("   <-- valid matmul, WRONG semantics" if B == D else ""))
    except RuntimeError:
        print(f"B={B}, D={D}: a @ b raises (shapes don't chain)  <-- the lucky case")
```
...
## 2. Split an interleaved Q/K feature axis

```python
def get_qk(x, nh, hd):
    """x: (B, S, D) where D = nh * 2 * hd.
       The feature axis is packed INTERLEAVED: [h0_q, h0_k, h1_q, h1_k, ...]
...
print(f"...and matches the reference? {torch.equal(qw, q_ref)}")
```

**The trap**: `chunk(2, dim=-1)` assumes the layout is `[all_Q ‖ all_K]`. This tensor is interleaved per head. The wrong version returns **the same shape**, raises **nothing**, and quietly feeds your attention scrambled vectors. Real codebases genuinely use both packings — [nanoGPT](https://github.com/karpathy/nanoGPT)'s `CausalSelfAttention` block-packs a fused QKV, other kernels interleave — which is why the right first move isn't writing code, it's establishing *how the producer packed the axis*. (External implementations change; check the version you're actually reading against rather than trusting a description like this one.)

Developed in full: [strides, views, and contiguity](01-strides-views-contiguity.qmd).
:::

---

## 3. Patchify (space-to-depth)

```python
img = torch.randn(N, C, H, W); patch = 2
# target shape: (N, C*patch*patch, H//patch, W//patch)
```
...
print(f"{tuple(img.shape)} -> {tuple(out.shape)}, matches PixelUnshuffle and inverts cleanly  ✓")
```
...
## 4. Sort one tensor by another tensor's keys

```python
data = torch.randn(4, 12, 3)          # (B, S, D)
keys = torch.randint(0, 256, (4, 12)) # (B, S)
# reorder data's S axis by each row's sort order of keys
```
...
print("take_along_dim, gather, and advanced indexing all agree  ✓")
```
...
## 5. Build a coordinate grid

```python
H = W = 4                     # square on purpose — see below
# want index_map of shape (H, W, 2) where index_map[i, j] == [i, j]
```
...
print(f"same shape? {index_map.shape == wrong.shape}   same values? {torch.equal(index_map, wrong)}")
```
...
And once you see that `meshgrid` is only sugar over two broadcast `arange`s, it stops being something to memorize:

```{python}
i = torch.arange(H).view(H, 1).expand(H, W)
j = torch.arange(W).view(1, W).expand(H, W)
assert torch.equal(torch.stack([i, j], -1), index_map)
print("pure broadcasting reproduces meshgrid('ij')  ✓")
```
:::

---

## 6. Masked sum of squares — which is cheaper on a GPU, and why?

```python
data = torch.randn(128); mask = torch.rand(128) < 0.7
a = (data * data * mask.type_as(data)).sum()
b = (data[mask] ** 2).sum()
```
...
print("sanitize-then-square:", (safe * safe).sum().item())
```
...
Two posts meet here: [the GPU cost model](05-how-the-gpu-runs-your-code.qmd) for the stall, [numerical stability](04-numerical-stability.qmd) for the gradient.
:::

---

## 7. Grouped-query attention: repeat KV heads to match Q heads

```python
k = torch.randn(B, n_kv_heads, S, hd)   # 2 kv heads
# want (B, n_q_heads, S, hd)            # 8 q heads
# query head h must attend to kv head h // (n_q // n_kv)
```
...
print(f"after reshape?                 {shares_allocation(llama, k)}   <- the copy lands here")
```
...
That "verify storage sharing explicitly rather than inferring it from syntax" habit is from [strides](01-strides-views-contiguity.qmd) — and it's what corrected this paragraph.
:::

---

## 8. Top-p (nucleus) sampling mask
...
print(f"naive   keeps {top_p_naive(lg, 0.9).sum().item()} token(s)  <- empty set")
```
...
assert mismatches == 0
print(f"2000 random (logits, p) pairs: {mismatches} mismatches  ✓")
```
:::

---

## 9. Ragged → padded, with a mask, no Python loop

```python
lens = torch.tensor([3, 1, 4])   # sequence lengths
flat = torch.arange(8).float()   # the sequences, concatenated
# want padded (3, 4) and a boolean mask (3, 4)
```
...
**The trap I fell into** is subtler, and it's worth flagging because it violates this series' own checklist. My first version wrote `T = int(lens.max())` and `torch.zeros(B, T)`. Both are quiet mistakes on a GPU: `int(...)` on a device tensor forces a **host synchronization** — precisely what [the GPU cost model post](05-how-the-gpu-runs-your-code.qmd) spends its length warning about — and `torch.zeros` without `device=` builds a CPU tensor. The version above takes `T` from a known capacity and uses `new_zeros` to inherit device and dtype. If you genuinely must derive `T` from the data in eager CUDA code, that sync is unavoidable, which is why production systems bucket sequence lengths, pad to fixed capacities, or use nested/jagged tensors instead.

**The follow-on**: this is exactly the mask that feeds attention — and if a zero-length row is represented by setting every logit to `-inf`, a naive softmax returns `nan` across that whole row unless the implementation defines an explicit empty-row policy. [Numerical stability](04-numerical-stability.qmd) works through the policy choices for that.
:::

---

## 10. Batched pairwise distances without the memory blow-up

```python
A = torch.randn(B, N, D); Bt = torch.randn(B, M, D)
# want (B, N, M) squared distances — the naive form OOMs
```
...
print("(agreeing within the stated floating-point tolerance, not bit-for-bit)")
```
...
- [ ] Output **shape** matches the spec — print it, don't assume it.
- [ ] **Layout assumption stated.** Problems 2 and 7 exist entirely to test this.
- [ ] `view` only when the requested shape is **stride-compatible** — not merely "when contiguous," since a non-contiguous tensor can still be view-compatible for a particular shape. `reshape` when a copy is acceptable, and when aliasing matters, check it: compare `a.untyped_storage().data_ptr()` (same allocation) rather than `a.data_ptr()` (same *first element* — two views into one buffer at different offsets disagree).
- [ ] No unintended **host-visible scalar or output-size dependency** in the deployed hot path — profile `.item()`, `nonzero`, boolean compaction, and Python control flow on tensor values under the configuration you actually ship (eager and compiled behave differently).
- [ ] Helper tensors created with matching `device=` and `dtype=`.
- [ ] **Numerical policy stated**: sanitize before unsafe arithmetic rather than after; define what a fully-masked softmax row *means*; and for expanded distances, inspect the scale of the negatives before clamping — small rounding negatives are a reasonable invariant repair, large ones are evidence the formulation or precision is wrong.
- [ ] Compared against a **suitable oracle** — a literal loop, a trusted built-in, a mathematical invariant, or an independently written implementation. (Not every solution above uses a loop: puzzle 1 checks against `torch.dot`, 3 against `PixelUnshuffle` plus a round-trip, 5 against a coordinate assertion, 8 against a reference implementation over 2,000 fuzzed inputs.) Prefer assertions to printed booleans, so a future edit can't quietly turn a `True` into a `False` while the prose keeps claiming success.
- [ ] **Your prose claims need checking too.** The recurring pattern while revising this series was that executable claims were easy to challenge and correct, while unsupported prose assumptions were much harder to notice — "this is zero-copy," "this always writes back," "this is the faster copy." Each of those is a one-line experiment. Run it.
