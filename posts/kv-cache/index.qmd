...
> **TL;DR** — Autoregressive generation re-runs the transformer over the *entire* prefix for every new token. Causal attention makes most of that work redundant: past tokens never look at future tokens, so their keys and values never change. Cache them, and each step processes **one token**. We build this from scratch on GPT-2, prove it's mathematically exact, break it in the three ways everyone breaks it, and measure a **[MEASURE]×** wall-clock speedup on a free Colab T4. Then we confront the two prices: the cache becomes the memory bottleneck of modern inference, and our simple implementation quietly copies gigabytes it already had.

*If you know attention already, skim to §3; if you build inference systems, §§5–8 are for you. Everything runs in [one Colab notebook](#) on a free T4.*
...
<!-- FIGURE: latency plot from Cell 6 -->
...
The rest of this post is the why, the how, the three bugs waiting for you, and the bill.
...
The KV cache eliminates the repeated *prefix* work in both terms. Be precise about what it doesn't do: the new token still attends over the full history, so cached decoding still costs $O(td)$ attention per step, $O(n^2 d)$ total — and generation stays stubbornly sequential, because token $t{+}1$ needs token $t$. The cache removes redundancy; it doesn't repeal autoregression. Totals:
...
**Why not cache Q too?** Symmetry suggests it, but look at dependency (1): past queries $q_{j<t}$ appear *nowhere* in any future step. A query is a search request — once token $t$'s attention row is computed, $q_t$ is dead. Causality means new tokens query the past; the past never queries the future. (In an encoder, where everyone attends to everyone and the sequence doesn't grow, none of this machinery applies. Caching is a *decoding* trick.)

Here is the entire idea, before any transformer machinery arrives:

```{python}
#| eval: false
def attention_one_token(q_t, K_cache, V_cache):
    """The KV cache in 12 lines. One head, one new token.

    q_t:     (d,)   query of the CURRENT token — computed fresh
    K_cache: (t, d) keys of ALL tokens so far   — reused from memory
    V_cache: (t, d) values of all tokens so far — reused from memory
    returns  (d,)   attention output for the current token
    """
    scores  = K_cache @ q_t / math.sqrt(q_t.numel())   # (t,)  one row of the attention matrix
    weights = torch.softmax(scores, dim=0)             # (t,)
    return weights @ V_cache                           # (d,)

# without a cache: recompute K_cache, V_cache from scratch every step.
# with a cache:    append one new (k_t, v_t) row, call this function. that's it.
```

Everything below is this function, scaled up to 12 layers and 12 heads, with the bookkeeping that scaling demands.
...
We'll write a ~120-line GPT-2 that loads real pretrained weights from Hugging Face, with a `forward` that optionally consumes and returns per-layer caches. (Architecture and weight-porting follow the well-known nanoGPT pattern.) First, pin down the symbols — every bug in §5 is a shape bug wearing a disguise:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, cache=None):
        ...
        if cache is not None:
            ...
            # ^ Simple, correct, and secretly expensive — §8 puts a number on it.
        ...
        if cache is not None and T != 1:
            raise ValueError("cached multi-token chunks need an explicit mask — see the post, §5")
...
**Cell 4 — look at what you built.** Before trusting the cache, inspect it. This ten-line habit — printing shapes before debugging math — has saved me more hours than any profiler:
...
def describe_cache(caches, max_layers=3):
    """caches: list of n_layer tuples (K, V), each (B, H, past, D_h)."""
    k0, _ = caches[0]
    B, H, past, Dh = k0.shape
    per_tok = 2 * len(caches) * H * Dh * k0.element_size()   # 2 = K and V
    print(f"{len(caches)} layers | {past} tokens cached | "
          f"{per_tok/1024:.0f} KiB/token | total {per_tok*past/2**20:.1f} MiB")
    for i, (k, v) in enumerate(caches[:max_layers]):
        print(f"  layer {i:02d}: K {tuple(k.shape)}  V {tuple(v.shape)}  {k.dtype}")

with torch.inference_mode():
    logits, caches = model(prompt)
describe_cache(caches)
# 12 layers | 7 tokens cached | 36 KiB/token | total 0.2 MiB
```

That 36 KiB/token is a number we'll derive — and worry about — in §7.

**Cell 5 — two generation loops**
...
**Cell 6b — 100-token integration check** (catches everything else — this single assert has ended more debugging sessions than any print statement I own):
...
## 5. Break it: the three bugs everyone writes

Explanations tell you what works. Debugging teaches you what matters. So let's write the bugs deliberately — each one below is a real mistake I've made or reviewed, each has a distinctive failure signature, and each is a one-line fix once you can name it.
...
**The advanced case, and why it's not pedantry.** Our guard rejects cached calls with $T > 1$. But feeding *multiple* new tokens against an existing cache is exactly what **speculative decoding** does (verify $k$ drafted tokens in one forward) and what **chunked prefill** does (process a long prompt in slices). For that, neither `is_causal=True` (top-left, wrong) nor `False` (no mask, also wrong: new token $past{+}0$ could attend to $past{+}1$) works. You need the lower-right-aligned mask, built by hand:

```{python}
#| eval: false
def causal_mask_with_past(query_len, past_len, device):
    """Causal mask for T>1 new tokens against past_len cached tokens.
    Query i sits at absolute position past_len+i and may attend to keys j <= past_len+i.
    Returns bool (query_len, past+query_len); True = may attend (SDPA convention).
    Broadcasts over (B, H)."""
    q_pos = past_len + torch.arange(query_len, device=device)[:, None]
    k_pos = torch.arange(past_len + query_len, device=device)[None, :]
    return k_pos <= q_pos

# usage inside attention, replacing the guard:
#   attn_mask = causal_mask_with_past(T, past, x.device)
#   y = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, is_causal=False)
```

(Recent PyTorch exposes this pattern as `CausalVariant.LOWER_RIGHT` in `torch.nn.attention.bias`; knowing why it exists is the point.)

### Bug 3 — batching + padding: deferred, deliberately

With batched prompts of different lengths you must left-pad (so every sequence's *last* token lands in the same column — that's the position whose logits you sample) and carry an explicit padding mask, because cached decode has no causal mask to hide behind and will happily attend to padding K/V as if they were content. Right-padding breaks in two places at once: the logits you gather at `[:, -1]` belong to padding, and the position offsets go wrong per-sequence. We dodge all of this with batch = 1 here; Part 3 solves it properly with per-sequence lengths, because at serving scale this bug graduates into a scheduling problem.

## 6. Measure it: benchmarks on a free T4

Numbers or it didn't happen. Protocol first — environment, then the race, then the plot:
...
|---|---|---|---|---|---|---|
| 16 | 400 | `[MEASURE]` s | `[MEASURE]` s | `[MEASURE]`× | `[MEASURE]` ms | `[MEASURE]` MiB |
| 128 | 400 | `[MEASURE]` s | `[MEASURE]` s | `[MEASURE]`× | `[MEASURE]` ms | `[MEASURE]` MiB |
| 512 | 400 | `[MEASURE]` s | `[MEASURE]` s | `[MEASURE]`× | `[MEASURE]` ms | `[MEASURE]` MiB |

Expect the speedup to *grow* with prompt length — exactly what the $O(n^2d^2) \to O(nd^2)$ analysis predicts, and worth verifying rather than asserting.

**The money plot** — note the cached cumulative curve includes the prefill, so both curves are honest end-to-end time:
...
| Llama-3-8B | 32 | 8×128 (GQA) | 128 | 1.0 GiB | **16 GiB** |
| Llama-3-70B | 80 | 8×128 (GQA) | 320 | 2.6 GiB | **40 GiB** |

*Per sequence — and that qualifier is doing heavy lifting. The cache is not a model property; it's a **model × sequence length × batch size** property. At batch 32, the Llama-3-8B 128k row becomes $32 \times 16 = 512$ GiB, before weights, activations, or allocator overhead. (Units are binary — KiB/GiB — since the values fall out as exact powers of two; sanity-check our toy: 36 KiB × 1024 ctx ≈ 37 MiB, which you can verify against `torch.cuda.memory_allocated()` before/after prefill.)*

Stare at the Llama-3-8B row: at 128k context, **the cache of one sequence matches the fp16 weights (16 GiB)**. A picture makes the crossover visceral, and this cell needs no GPU:

```{python}
#| eval: false
ctx = np.arange(1, 128*1024, 1024)
kv  = kv_bytes_per_token(32, 8, 128) * ctx / 2**30          # Llama-3-8B, GiB
plt.figure(figsize=(7, 4))
plt.axhline(16, ls="--", c="gray", label="fp16 weights (16 GiB)")
plt.plot(ctx/1024, kv,     label="KV cache, batch=1")
plt.plot(ctx/1024, 8*kv,   label="KV cache, batch=8")
plt.xlabel("context length (k tokens)"); plt.ylabel("memory (GiB)")
plt.legend(); plt.title("Llama-3-8B: when the cache outgrows the model")
plt.tight_layout(); plt.savefig("figures/crossover.png", dpi=180)
```

This single table explains half of modern inference research:

- **MQA / GQA** (1 or few KV heads shared across query heads): shrink $n_{\text{kv-heads}}$ — that's the 4× gap between the Llama-2 and Llama-3 rows, bought at training time.
...
- **PagedAttention** (vLLM): stop allocating the cache contiguously; page it like an OS pages RAM — which also unlocks *prefix sharing* (one system prompt, cached once, referenced by every request). Part 3.
...
which for our 128-prompt, 400-token benchmark is $36{,}864 \times (528^2 - 128^2)/2 \approx$ **4.5 GiB of data the GPU already had**. The notebook prints your exact figure:

```{python}
#| eval: false
n0, n1, bpt = 128, 528, kv_bytes_per_token(12, 12, 64)
print(f"~{bpt * (n1**2 - n0**2) / 2 / 2**30:.1f} GiB copied by torch.cat during one generation")
```

It's *fine here* — GPT-2 is small and the T4 shrugs. It does not stay fine. The grown-up version preallocates once and writes each new token into its slot:
...
And the bridge to the library everyone actually uses: Hugging Face's `model.generate(..., use_cache=True)` runs exactly the loop we wrote, carrying our list-of-(K,V)-tuples under the name `past_key_values` inside a `DynamicCache` — with `StaticCache` as the preallocated variant we just built. Swap it in on our benchmark and you should see the same speedup; the from-scratch and production paths are the same idea wearing different APIs.

What we still haven't solved is everything that makes serving hard: our static cache reserves `max_len` for one sequence whether it uses it or not; requests of different lengths fragment memory; finished sequences strand their slots; identical system prompts get cached once per request instead of once. "Store K and V" was never the hard part — *allocating, sharing, and evicting* them under a hundred concurrent requests is, and that's precisely the problem PagedAttention and continuous batching exist to solve. Part 3 builds it.

## 9. Prefill vs decode: one model, two workloads
...
| Batch size helps? | somewhat — long prompts already saturate the GPU | massively — amortizes the streaming |
| Latency metric | TTFT (time to first token) | TPOT / inter-token latency |

A decode step must read every weight and the whole cache from GPU DRAM to produce *one* token — arithmetic intensity near 1 FLOP/byte, while a T4 (≈65 fp16 TFLOPS against ≈320 GB/s) needs roughly 200 to stay busy. The GPU idles on memory. Two honest framings before the sequel-baiting: batching helps *both* phases, just asymmetrically — prefill on a long prompt is already decent matmul work for one request, while decode degenerates to matrix-vector and desperately needs company. And FlashAttention (Part 2) earns its fame in *prefill and training*, where a $T \times T$ score matrix gets materialized; in one-token decode there is no square matrix to avoid, and the fight moves to streaming the weights and the cache — which is Part 3's territory, plus a decode-specific kernel family (Flash-Decoding) we'll meet at the end of Part 2.

## 10. What's next

You now have a model that generates efficiently for *one* sequence, a proof that the optimization is free, three bugs you'll recognize on sight, and a cache whose size you can predict to the byte. **Part 2** opens the attention kernel itself: why materializing the $T\times T$ score matrix is the real crime of prefill, and how FlashAttention computes exact attention without ever writing it down. Same repo, same MiniGPT.
...
- Kwon et al., [*Efficient Memory Management for LLM Serving with PagedAttention*](https://arxiv.org/abs/2309.06180), SOSP 2023 — where §8's open problems get solved.

**Docs & code**

- [Hugging Face: KV cache strategies](https://huggingface.co/docs/transformers/en/cache_explanation) — read this next; it's our §8 exposed as an API (`DynamicCache`, `StaticCache`, `past_key_values`).
...
- [kipply, *Transformer Inference Arithmetic*](https://kipply.com/blog/transformer-inference-arithmetic/), 2022 — the classic back-of-envelope treatment; §§6–7 here are its measured companion.
